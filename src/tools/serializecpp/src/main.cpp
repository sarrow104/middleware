...
#include "../include/serializecpp_class.h"
using namespace std;
...
void test_2()
{
	...
	lstruct.arg2 = 15;
	lstruct.arg3 = 'x';
	serializecpp::push( &lsbuf, lstruct );
	/* 数组 */
	...
	serializecpp::push( &lsbuf, larray , 3 );
	...
	unserializecpp::pop( &lsbuf2, lstruct2 );
	...
	unserializecpp::pop( &lsbuf2, larray2, 3 );
	for( uint32_t i=0;i<3;++i)
	{
...
void test_3()
{
	...
	serializecpp::push( &lsbuf, lcarray , 13 );
	const char* lcstr = "helloworld";
	serializecpp::push( &lsbuf, lcstr , sizeof("helloworld") );


	std::string lstr ="my open code!";
	serializecpp::push( &lsbuf, lstr );

	std::vector<int> lvec(10,38);
	serializecpp::push( &lsbuf, lvec );
	...
	serializecpp::push( &lsbuf, lunset );

	/* 反序列化 */
	serializecpp_buffer lsbuf2(lsbuf.get_buffer(),lsbuf.get_uselen());

	char lcarray2[13];
	unserializecpp::pop( &lsbuf2, lcarray2, 13 );
	...
	char lcstr2[128];
	unserializecpp::pop( &lsbuf2, lcstr2, 128 );
	cout<<"lcstr";
	cout<< ( ( strcmp( lcarray, lcarray2) == 0) ? "==" : "!=");
	cout<<"lcstr2"<<endl;

	std::string lstr2;
	unserializecpp::pop( &lsbuf2, lstr2 );
	cout<<"lstr";
	cout<< ( ( lstr == lstr2) ? "==" : "!=");
	cout<<"lstr2"<<endl;

	std::vector<int> lvec2;
	unserializecpp::pop( &lsbuf2, lvec2 );
	cout<<"lvec";
	cout<< ( ( lvec == lvec2) ? "==" : "!=");
	cout<<"lvec2"<<endl;

	std::set<int> lset2;
	unserializecpp::pop( &lsbuf2, lset2 );
	cout<<"lset";
	cout<< ( ( lset == lset2) ? "==" : "!=");
	cout<<"lset2"<<endl;

	std::map<int,int> lmap2;
	unserializecpp::pop( &lsbuf2, lmap2 );
	cout<<"lmap";
	cout<< ( ( lmap == lmap2) ? "==" : "!=");
	cout<<"lmap2"<<endl;

	std::unordered_set<int> lunset2;
	unserializecpp::pop( &lsbuf2, lunset2 );
	cout<<"lunset";
	cout<< ( ( lunset == lunset2) ? "==" : "!=");
	cout<<"lunset2"<<endl;
}
