...
#ifndef PROTOCOL_HEAD_HANDLE_BASE_H
#define PROTOCOL_HEAD_HANDLE_BASE_H
#include <cstdint>
#include <vector>
using std::vector;
#include "check_crc.h"
...
public:
	...
	enum POS
	{
		//HEAD_SERVER_MAGIC_POS					= 0,		/* 魔数 */
		HEAD_SERVER_PROTOCOL_LEN_POS			= 0,//HEAD_SERVER_MAGIC_POS + sizeof( HEAD_SERVER_MAGIC_TYPE ),
	...
	//	HEAD_SESSIONTYPE_POS					= HEAD_CLOSESOCKET_COUNT_POS + sizeof( HEAD_CLOSESOCKET_COUNT_TYPE ),
		HEAD_GROUPID_POS						= HEAD_CLOSESOCKET_COUNT_POS + sizeof( HEAD_CLOSESOCKET_COUNT_TYPE ),/*HEAD_SESSIONTYPE_POS + sizeof( HEAD_SESSIONTYPE_TYPE ),*/
	...
	/*void set_mgic_num()
	{
		*( (HEAD_SERVER_MAGIC_TYPE*)&( m_data[ HEAD_SERVER_MAGIC_POS ] ) ) = (uint32_t)(-1);
	}*/
	/* 获取mgic_num 魔数 */
	//GET_HEAD( mgic_num , HEAD_SERVER_MAGIC_TYPE , HEAD_SERVER_MAGIC_POS  )

	/* 获取socket 关闭次数 */
	GET_HEAD( closesocket_count , HEAD_CLOSESOCKET_COUNT_TYPE , HEAD_CLOSESOCKET_COUNT_POS  )

	/* 获取session type */
	//GET_HEAD( sessiontype , HEAD_SESSIONTYPE_TYPE , HEAD_SESSIONTYPE_POS  )
...
public:
	enum POS
	{
		HEAD_DATA_BYTE_POS			= server_head::POS::END_POS,
	...
	{
		*( (HEAD_DATA_BYTE_TYPE*)&( m_data[ POS::HEAD_DATA_BYTE_POS ] ) ) = aibadyvalues + ( END_POS - server_head::END_POS) ;
	...
	void set_crc()
	{
		get_crc() = 
			...
			get_data_byte() - (POS::HEAD_USER_ID_POS - server_head::POS::END_POS)  
			);
	}
	/* 检查crc */
	bool check_crc()
	...
	{
		return get_server_protocol_len() - POS::END_POS;
	}
	...
	void swap( protocol_server_head& ap )
	{
		std::swap( m_data , ap.m_data  );
		std::swap( m_size , ap.m_size  );
		SH_reset( m_data , m_size );
	}

	/* 只能拷贝头 */
	protocol_server_head& operator=( protocol_server_head& ap )
	{
		memcpy( m_data , ap.m_data , END_POS );
		set_data_byte( 0 );
		SH_set_mass_send_arr();
		return *this;
	}

	static protocol_server_head* alloc_head()
	{
		protocol_server_head* lret = new protocol_server_head();
		char* lbuf = new char[ END_POS ]; 
		lret->reset( lbuf , END_POS );
		lret->set_data_byte( 0 );
		lret->SH_set_mass_send_arr();
		return lret;
	}

	static void release_head( protocol_server_head* ap )
	{
		delete [] ap->m_data;
		delete ap;
	}
...
public:
	...
	void set_data_byte( uint32_t aibadyvalues )
	{
		*( (HEAD_DATA_BYTE_TYPE*)&( m_data[ POS::HEAD_DATA_BYTE_POS ] ) ) = aibadyvalues + END_POS;
	}
	...
	uint32_t get_protocol_body_len()
	{
		return get_data_byte() - POS::END_POS;
	}
...
#endif //PROTOCOL_HEAD_HANDLE_BASE_H