...
namespace middleware_base{
  ...
  public:
    ...
    uint16_t m_port;                                            /* 端口号 */
    boost::function<bool(char*, uint32_t&)>* m_callbackfun;     /* 回调 */
    ...
    socket_asio_server_arg(uint32_t aithreadsize) :
      m_thread_pos(0)
    {
      m_threadmaxsize = aithreadsize;
      m_middlewarearr = new middleware_base*[aithreadsize];
      m_callbackfun = new boost::function<bool(char*, uint32_t&)>[aithreadsize];
    }
